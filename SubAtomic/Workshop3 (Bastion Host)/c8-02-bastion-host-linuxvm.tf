...
resource "azurerm_public_ip" "bastion_host_publicip" {
    name = "${local.resource_name_prefix}-bastion-host-pubicip"
...
resource "azurerm_network_interface" "bastion_host_linuxvm_nic" {
  name = "${local.resource_name_prefix}-baastion-host-linuxvm-nic"
  location = azurerm_resource_group.rg.location
  resource_group_name = azurerm_resource_group.rg.name
...
# Resource-3: Azure Linux VM - Bastion Host
resource "azurerm_linux_virtual_machine" "bastion_host_linuxvm" {
  ...
  network_interface_ids = [azurerm_network_interface.bastion_host_linuxvm_nic]
  admin_ssh_key {
    username = "azureuser"
